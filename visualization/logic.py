...
class OverlayRenderer:
    ...
    def draw_frenet_panel(
        ...
        # kappa_profile: [[s_i, kappa_i], ...] (control/node.py, độ cong CSV
        # THẬT dọc quãng đường phía trước, s tương đối từ vị trí hiện tại) —
        # nếu có, tích phân từng đoạn để bend() vẽ đúng HÌNH DẠNG cua thật
        # (khác cung tròn bán kính không đổi bên dưới, chỉ đúng khi cua đều).
        # Bag cũ/không dùng GPS route -> rỗng, rơi về công thức cung tròn cũ.
        _bend_grid = None
        kappa_profile = frenet.get("kappa_profile") or []
        if len(kappa_profile) >= 2:
            prof_s = np.asarray([p[0] for p in kappa_profile], dtype=float)
            prof_k = np.asarray([p[1] for p in kappa_profile], dtype=float)
            if prof_s[-1] > prof_s[0]:
                s_fine = np.linspace(prof_s[0], prof_s[-1], max(4 * len(prof_s), 32))
                k_fine = np.interp(s_fine, prof_s, prof_k)
                ds_fine = np.diff(s_fine, prepend=s_fine[0])
                phi = np.cumsum(k_fine * ds_fine)          # phi(s) = ∫kappa ds
                X = np.cumsum(np.sin(phi) * ds_fine)        # X(s) = ∫sin(phi) ds
                Y = np.cumsum(np.cos(phi) * ds_fine)        # Y(s) = ∫cos(phi) ds
                _bend_grid = (s_fine, phi, X, Y)

        def bend(d_val: float, s_val: float) -> tuple[float, float]:
            """Chiếu (d, s) Frenet-thẳng lên hình dạng cua — dùng kappa_profile
            (tích phân từng đoạn theo kappa(s) thật, _bend_grid) nếu có, không
            thì cung tròn bán kính 1/kappa không đổi (panel: +kappa = cua
            phải) — cả 2 quy về (d_val, s_val) y hệt khi kappa=0/không profile."""
            if _bend_grid is not None:
                s_fine, phi, X, Y = _bend_grid
                s_c = float(np.clip(s_val, s_fine[0], s_fine[-1]))
                phi_i = float(np.interp(s_c, s_fine, phi))
                x_i = float(np.interp(s_c, s_fine, X))
                y_i = float(np.interp(s_c, s_fine, Y))
                c, sn = np.cos(phi_i), np.sin(phi_i)
                return x_i + d_val * c, y_i - d_val * sn
            if abs(kappa) < 1e-6:
                return d_val, s_val
            ks = kappa * s_val
    ...
    def draw_curve_map_panel(
        self,
        canvas: np.ndarray,
        route_xy: np.ndarray,
        viz: dict[str, Any],
        curve_zones: list[tuple[float, float]] | None,
        px0: int,
        py0: int,
        panel_w: int,
        panel_h: int,
        zoom_zone: bool = True,
        zone_margin_m: float = 2.0,
    ) -> None:
        """Panel Frenet ở CHẾ ĐỘ CUA (curve_frame payload từ control_node:
        _planner_tick_curve): vẽ trong frame mét local của tuyến CSV, xe/path/
        obstacle đặt đúng toạ độ map — KHÔNG còn ego-frame như
        draw_frenet_panel. zoom_zone=True (mặc định): khung nhìn là ĐÚNG đoạn
        cua đang chạy — từ waypoint ĐẦU tới waypoint CUỐI của curve zone chứa
        vị trí xe (± zone_margin_m chỉ để path/xe không bị cắt sát viền panel),
        không phải toàn bộ 551m tuyến. zoom_zone=False: vẽ toàn tuyến (dùng để
        xem tổng quan vị trí cua nằm ở đâu trên cả route)."""
        h, w = canvas.shape[:2]
        panel_w = max(120, min(panel_w, w - px0 - 1))
        panel_h = max(140, min(panel_h, h - py0 - 1))

        # Nền + viền cam (curve mode luôn là GPS+encoder) + banner.
        cv2.rectangle(canvas, (px0, py0), (px0 + panel_w, py0 + panel_h),
                      (255, 255, 255), -1)
        cv2.rectangle(canvas, (px0, py0), (px0 + panel_w, py0 + panel_h),
                      (0, 140, 255), 3)
        cv2.putText(canvas, "Frenet (CSV frame)", (px0 + 4, py0 + 12),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.30, (70, 70, 70), 1)
        banner = "CURVE MODE"
        (tw, th), _ = cv2.getTextSize(banner, cv2.FONT_HERSHEY_SIMPLEX, 0.42, 2)
        bx0, by0 = px0 + panel_w - tw - 14, py0 + 2
        ...
        # s dọc polyline — cùng cách draw_gps_panel tính để khớp biên zone.
        seg_len = np.linalg.norm(np.diff(route_xy, axis=0), axis=1)
        s_cum = np.concatenate([[0.0], np.cumsum(seg_len)])

        # Vùng nhìn: toàn tuyến (mặc định, "từ điểm đầu tới điểm cuối CSV")
        # hoặc riêng curve zone đang chứa xe khi zoom_zone.
        view_pts = route_xy
        route_s = viz.get("route_s")
        if zoom_zone and curve_zones and route_s is not None:
            for a, b in curve_zones:
                if a <= float(route_s) <= b:
                    mask = (s_cum >= a - zone_margin_m) & (s_cum <= b + zone_margin_m)
                    if mask.sum() >= 2:
                        view_pts = route_xy[mask]
                    break

        x_min, y_min = view_pts.min(axis=0)
        x_max, y_max = view_pts.max(axis=0)
        ...
        # Tuyến CSV (tím, như panel GPS) + tô cam các curve zone.
        ...
        # Candidate paths (xám mờ) rồi optimal path (cyan) — map-local xy.
        for cand in viz.get("candidate_paths_xy") or []:
            cpts = [to_px(float(x), float(y)) for x, y in cand]
            if len(cpts) >= 2:
                cv2.polylines(canvas, [np.array(cpts, dtype=np.int32)],
                              False, (150, 150, 150), 1, cv2.LINE_AA)
        opt = viz.get("optimal_path_xy")
        if opt:
            opts = [to_px(float(x), float(y)) for x, y in opt]
            if len(opts) >= 2:
                cv2.polylines(canvas, [np.array(opts, dtype=np.int32)],
                              False, (255, 255, 0), 2, cv2.LINE_AA)
                cv2.circle(canvas, opts[-1], 3, (255, 255, 0), -1)

        # Obstacle (đỏ) — đã transform sang map-local ở control_node.
        for obs in viz.get("obstacles_xy") or []:
            op = to_px(float(obs[0]), float(obs[1]))
            cv2.circle(canvas, op, 5, (40, 40, 220), -1)
            cv2.circle(canvas, op, 7, (120, 120, 255), 1)

        # Điểm lookahead pure pursuit (hồng).
        la = viz.get("lookahead_xy")
        if la:
            lp = to_px(float(la[0]), float(la[1]))
            cv2.circle(canvas, lp, 5, (220, 0, 220), -1)
            cv2.circle(canvas, lp, 8, (255, 180, 255), 1, cv2.LINE_AA)

        # Xe: chữ nhật xoay theo car_yaw (map CCW+ -> màn hình y xuống dưới).
        car_xy = viz.get("car_xy")
        if car_xy:
            cp = to_px(float(car_xy[0]), float(car_xy[1]))
            yaw = float(viz.get("car_yaw", 0.0))
            fwd = np.array([np.cos(yaw), -np.sin(yaw)])   # px frame: y đảo
            right = np.array([np.sin(yaw), np.cos(yaw)])
            center = np.array([float(cp[0]), float(cp[1])])
            half_len, half_wid = 7.0, 5.0
            corners = np.array(
                [
            ...
            cv2.fillPoly(canvas, [corners], (0, 140, 255))
            cv2.polylines(canvas, [corners], True, (0, 90, 170), 1, cv2.LINE_AA)
            tip = center + fwd * (half_len + 12.0)
            cv2.arrowedLine(canvas, (int(center[0]), int(center[1])),
                            (int(tip[0]), int(tip[1])),
                            (0, 90, 170), 1, cv2.LINE_AA, tipLength=0.35)

        # Số liệu route state hiện tại.
        d_v = viz.get("route_d")
        psi_v = viz.get("route_psi_err")
        sig_v = viz.get("route_sigma_d")
        parts = []
        if route_s is not None:
            parts.append(f"s={float(route_s):.1f}m")
        if d_v is not None:
            parts.append(f"d={float(d_v):+.2f}m")
        if psi_v is not None:
            parts.append(f"psi={np.degrees(float(psi_v)):+.1f}deg")
        if sig_v is not None:
            parts.append(f"sigma_d={float(sig_v):.2f}m")
        cv2.putText(canvas, "  ".join(parts) if parts else "no route state",
                    (px0 + 4, py0 + panel_h - 4),
